...
const buildStatusMessage = (order: { id?: string; name?: string; status?: string }) => {
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const result = await sendWA(phone, buildStatusMessage({ id: orderId, name, status }));
    return res.status(200).json({ ...result, orderId: orderId || "unknown" });
  } catch (error) {
    console.error("notify-status API error:", error);
